...
function AdmissionsContact({ sections, contact, admissions }) {
    ...
    <section data-cms-section="home-admissions" className="bg-cream py-12 md:py-16 relative">
      <div className="container-x">
        <SectionHeader
          ...
          title={sections?.admissionsTitle || 'A personal conversation. Not a funnel.'}
          ...
          eyebrowPath="sections.admissionsEyebrow"
          titlePath="sections.admissionsTitle"
          subtitlePath="sections.admissionsSubtitle"
        />

        <div className="mt-8 md:mt-16 grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-14 lg:gap-16">
        {/* Admissions process */}
        <div>
          <p className="font-caps text-[0.65rem] text-gold tracking-[0.22em] mb-5" data-cms-path="admissions.processEyebrow">
            {admissions?.processEyebrow || 'The Process'}
          </p>
          <div className="space-y-6 max-w-md">
            {((admissions?.steps && admissions.steps.length > 0) ? admissions.steps : [
              { n: '01', title: 'Application', body: 'Tell us about your work and what you want to learn.' },
              { n: '02', title: 'Conversation', body: 'A personal call to discuss fit and expectations.' },
              ...
              <div key={s.n || i} className="flex gap-5 border-b border-navy/10 pb-5 last:border-0 group">
                <p className="font-display text-gold text-[1.8rem] leading-none flex-shrink-0 group-hover:scale-110 transition-transform origin-left" data-cms-path={`admissions.steps.${i}.n`}>{s.n}</p>
                <div>
                  <h4 className="font-display text-navy text-[1.1rem] leading-tight" data-cms-path={`admissions.steps.${i}.title`}>{s.title || s.t}</h4>
                  <p className="font-editorial text-navy/70 text-base leading-relaxed mt-1" data-cms-path={`admissions.steps.${i}.body`}>{s.body || s.b}</p>
                </div>
              </div>
            ))}
          </div>
...
export default function Home() {
  ...
  const admissions = home?.admissions || {};
    ...
    <div>
      ...
      <div className="bg-navy-deep">
        <div className="container-x pt-10 md:pt-20">
          <SectionHeader
            chapter="Chapter III"
            eyebrow={sections?.faculty?.eyebrow || 'Faculty'}
            title={sections?.faculty?.title || 'Practitioner-educators who'}
            accent={sections?.faculty?.accent || 'do the work.'}
            tone="dark"
      ...
      <section data-cms-section="home-testimonials" className="bg-navy-deep text-cream py-12 md:py-16 relative overflow-hidden">
        ...
        <div className="container-x relative">
          <SectionHeader
            ...
            title={sections.testimonialsTitle || 'The judgement shows up in their work.'}
            align="center"
            tone="dark"
            eyebrowPath="sections.testimonialsEyebrow"
            titlePath="sections.testimonialsTitle"
          />
